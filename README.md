**NOTE:**
This is not a functional/complete mod - the script & documentation is completely generated by Google Gemini, so it is likely garbage. HOWEVER, it does describe the intended interactions, and may serve as an outline for mod development.
...
Security: All gold math and item transfers are handled Server-Side. 
This prevents clients from "spoofing" gold amounts or duplicating currency.

Sync Logic: The system includes a "Drift Check." 
If a player initiates a command but the target moves beyond the allowed units before the menu selection is finished, the action is cancelled.
